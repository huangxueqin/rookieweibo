...
public class SlideTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    public SlideTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabGravity = ta.getInt(R.styleable.SlideTabLayout_tab_gravity, GRAVITY_CENTER);
        mTabMode = ta.getInt(R.styleable.SlideTabLayout_tab_mode, MODE_FIT_SCREEN);
        mTabStyle = ta.getInt(R.styleable.SlideTabLayout_tab_style, STYLE_PLAIN_TEXT);
        mTabWidth = ta.getDimensionPixelSize(R.styleable.SlideTabLayout_tab_width, 0);
        ta.recycle();
